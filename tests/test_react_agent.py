...
class ReActAgent:
    ...
    async def generate_answer(self, question: str, documents: List[Dict[str, Any]]) -> str:
        ...
        try:
            answer = await chain.ainvoke({"question": question, "documents": documents})
...
@pytest.mark.asyncio
async def test_error_handling(react_agent, mock_retriever):
    mock_retriever.aget_relevant_documents.side_effect = Exception("Test error")
    
    with pytest.raises(Exception):
        await react_agent.run("test query")